    ...
    match *o_am_conn_core {
        ...
        Some(ref mut am_conn_core) => {
            ...
            request.push(Part::new(
                ...
                Argument::ReadLobRequest(ReadLobRequest::new(locator_id, offset, length_to_read)),
            ));
...
pub fn fetch_a_char_lob_chunk(
    ...
    match *o_am_conn_core {
        ...
        Some(ref mut am_conn_core) => {
            // build the request, provide StatementContext and length_to_read
            let mut request = Request::new(RequestType::ReadLob, 0);
            let char_length_to_read = {
                let guard = am_conn_core.lock()?;
                cmp::min(
                    (*guard).get_lob_read_length() as u64,
                    length_c - acc_char_length,
                ) as i32
            };
            let char_offset = acc_char_length + 1;
            request.push(Part::new(
                PartKind::ReadLobRequest,
            ...
            trace!(
                "Sending ReadLobRequest with char_offset = {} and char_length_to_read = {}",
                char_offset,
                char_length_to_read
            );
