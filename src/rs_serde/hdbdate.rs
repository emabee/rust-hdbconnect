...
pub fn longdate_from_str(s: &str) -> Result<LongDate, SerializationError> {
    let funcs: Vec<fn(&str) -> Result<LongDate, ()>> = vec![from_naivedt_string_full,
...
fn from_naivedt_string_day(s: &str) -> Result<LongDate, ()> {
    ...
    match NaiveDate::parse_from_str(s, "%Y-%m-%d") {
        Ok(ndt_parsed) => {
            let ld =
                try!(LongDate::from_ymd(ndt_parsed.year(), ndt_parsed.month(), ndt_parsed.day())
                    .or(Err(())));
...
fn from_utc_string(s: &str) -> Result<LongDate, ()> {
    ...
    match DateTime::parse_from_rfc3339(s) {
        Ok(dt) => {
            ...
            let ld = try!(LongDate::from_ymd_hms_n(ndt.year(),
                                                   ndt.month(),
                                                   ndt.day(),
                                                   ndt.hour(),
                                                   ndt.minute(),
