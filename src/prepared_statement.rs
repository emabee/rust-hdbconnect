...
pub struct PreparedStatement {
    ...
    #[allow(dead_code)]                  // FIXME if support for reroute is required
    o_table_location: Option<Vec<i32>>,
    o_par_md: Option<ParameterMetadata>, // optional, because there will not always be parameters
    #[allow(dead_code)]                  // FIXME add an accessor method for the ResultSetMetadata
    ...
    pub fn prepare(conn_ref: ConnRef, stmt: String, auto_commit: bool) -> DbcResult<PreparedStatement> {
        ...
        while reply.parts.0.len() > 0 {
            match reply.parts.pop_arg() {
                Some(Argument::ParameterMetadata(par_md)) => {o_par_md = Some(par_md);},
                Some(Argument::StatementId(id)) => {o_stmt_id = Some(id);},
                Some(Argument::StatementContext(stmt_ctx)) => {o_stmt_ctx = Some(stmt_ctx);},
                Some(Argument::TransactionFlags(vec)) => {o_ta_flags = Some(vec)},
                Some(Argument::TableLocation(vec_i)) => {o_table_location = Some(vec_i);},
                Some(Argument::ResultSetMetadata(rs_md)) => {o_rs_md = Some(rs_md);},
        ...
        if let Some(vec) = o_ta_flags {
            ...
            for ta_flag in vec { try!(conn_core.set_transaction_state(ta_flag)); }
        }
        ...
        Ok(PreparedStatement {
            ...
            o_batch: match &o_par_md {&Some(_) => Some(Vec::<ParameterRow>::new()), &None => None},
            o_par_md: o_par_md,
            o_rs_md: o_rs_md,
