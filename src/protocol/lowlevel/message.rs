...
impl Message {
    ...
    pub fn send_and_receive(&mut self, o_rs: &mut Option<&mut ResultSet>, stream: &mut TcpStream)
    -> DbcResult<Message> {
        ...
        let mut msg = try!(parse(o_rs, &mut rdr));
        try!(msg.assert_no_error());
        Ok(msg)
    }


    fn serialize(&self, w: &mut io::Write) -> DbcResult<()> {
    ...
    fn assert_no_error(&mut self) -> DbcResult<()> {
        ...
        for ref mut seg in &mut self.segments {
            if let Some(idx) = util::get_first_part_of_kind(PartKind::Error, &seg.parts) {
                ...
                let vec = match errpart.arg {
                    Argument::Error(v) => v,
                    _ => return Err(DbcError::ProtocolError("Inconsistent error part found".to_string())),
                };
                let err = DbcError::DbMessage(vec);
                warn!("{}",err);
                return Err(err);
            }
        }
...
///
fn parse(o_rs: &mut Option<&mut ResultSet>, rdr: &mut io::BufRead) -> DbcResult<Message> {
    ...
    let mut msg = Message::new(session_id,packet_seq_number);
    for _ in 0..no_of_segs { msg.segments.push(try!(segment::parse(o_rs, rdr))); }
    Ok(msg)
}
