...
pub mod factory {
    ...
    pub fn parse_from_reply(p_typecode: u8, nullable: bool, conn_ref: &ConnRef,
        ...
        match typecode {
            ...
            25 => Ok(TypedValue::CLOB(try!(parse_clob_from_reply(conn_ref, rdr)))),
            ...
            153 => Ok(TypedValue::N_CLOB(try!(parse_nullable_clob_from_reply(conn_ref, rdr)))),
    ...
    fn parse_nullable_length_and_binary(rdr: &mut io::BufRead) -> PrtResult<Option<Vec<u8>>> {
        ...
        Ok(Some(try!(util::parse_bytes(len, rdr))))                                 // B (varying)
    }
