...
impl OptionValue {
    pub fn serialize(&self, w: &mut io::Write) -> PrtResult<()> {
        ...
        match *self {                                       // variable
            ...
            OptionValue::STRING(ref s)       => {try!(serialize_length_and_string(s,w))},
            OptionValue::BSTRING(ref v)      => {try!(serialize_length_and_bytes(v,w))},
        }
        Ok(())
    }
    ...
    fn parse_value(typecode: u8, rdr: &mut io::BufRead) -> PrtResult<OptionValue> { match typecode {
        ...
        _  => Err(PrtError::ProtocolError(format!("OptionValue::parse_value() not implemented for type code {}",typecode))),
    }}
}
