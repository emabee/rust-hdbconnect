...
impl ClobFromDB {
    pub fn new(conn_ref: &ConnRef, is_data_complete: bool,
               length_c: u64, length_b: u64, char_count: u64,
    ...
    fn fetch_next_chunk(&mut self) -> PrtResult<()> {
        let (reply_data, reply_is_last_data, server_processing_time)
            = try!(fetch_a_lob_chunk(&self.o_conn_ref, self.locator_id, self.length_b, self.char_count));
...
pub fn parse_nullable_clob_from_reply(conn_ref: &ConnRef, rdr: &mut io::BufRead) -> PrtResult<Option<CLOB>> {
    ...
    match is_null {
        ...
        false => {
            ...
            Ok(Some(CLOB::FromDB(ClobFromDB::new(conn_ref, is_last_data, length_c, length_b, char_count, locator_id, s))))
        }
    }
}
...
-> PrtResult<(Vec<u8>,bool, i32)> {
    ...
    let (conn_ref, length_to_read) = match *o_conn_ref {
        None => {return Err(prot_err(
            "LOB is not complete, but fetching more chunks is no more possible (connection not available)"));},
        Some(ref conn_ref) => {
