...
impl OptionValue {
    ...
    pub fn parse(rdr: &mut io::BufRead) -> DbcResult<OptionValue> {
        let value_type = try!(rdr.read_u8());                                       // U1
        OptionValue::parse_value(value_type, rdr)
    }

    fn parse_value(typecode: u8, rdr: &mut io::BufRead) -> DbcResult<OptionValue> { match typecode {
        ...
        _  => Err(DbcError::ProtocolError(format!("OptionValue::parse_value() not implemented for type code {}",typecode))),
    }}
}
