...
impl fmt::Debug for PartAttributes {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if self.bits == 0 {
            ...
            if self.has_next_packet()     { b = try!(w_and(b,f)); try!(write!(f, "HAS_NEXT_PACKET")); };
            if self.is_first_packet()     { b = try!(w_and(b,f)); try!(write!(f, "IS_FIRST_PACKET_IN_A_SEQUENCE")); };
            if self.row_not_found()       { b = try!(w_and(b,f)); try!(write!(f, "ROW_NOT_FOUND")); };
            if self.is_resultset_closed() { try!(w_and(b,f));     try!(write!(f, "RESULTSET_CLOSED")); };
            write!(f, ")")
        }
    }
}
