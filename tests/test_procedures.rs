...
fn very_simple_procedure(connection: &mut Connection) -> HdbResult<()> {
    ...
    test_utils::multiple_statements(connection,
                                    vec!["CREATE PROCEDURE TEST_PROCEDURE LANGUAGE SQLSCRIPT \
                                          SQL SECURITY DEFINER AS BEGIN SELECT CURRENT_USER \
                                          \"current user\" FROM DUMMY;END"])?;
...
fn procedure_with_out_resultsets(connection: &mut Connection) -> HdbResult<()> {
    ...
    test_utils::multiple_statements(connection,
        ...
        CREATE  PROCEDURE GET_PROCEDURES( OUT \
                                          procedures TABLE(schema_name NVARCHAR(256), name \
                                          NVARCHAR(256)) ,OUT hana_dus TABLE(du NVARCHAR(256), \
                                          vendor NVARCHAR(256)), OUT other_dus TABLE(du \
                                          NVARCHAR(256), vendor NVARCHAR(256)) ) AS BEGIN \
                                          procedures = SELECT schema_name AS schema_name, \
                                          procedure_name AS name FROM PROCEDURES WHERE \
                                          IS_VALID = 'TRUE'; \
                                               \
                                          hana_dus = select delivery_unit as du, vendor from \
                                          _SYS_REPO.DELIVERY_UNITS where delivery_unit like \
                                          'HANA%'; other_dus = select delivery_unit as du, \
                                          vendor from _SYS_REPO.DELIVERY_UNITS where not \
...
fn procedure_with_secret_resultsets(connection: &mut Connection) -> HdbResult<()> {
    ...
    test_utils::multiple_statements(connection,
        ...
        CREATE  PROCEDURE \
                                          ...
                                          schema_name AS schema_name, procedure_name AS name \
                                          FROM PROCEDURES WHERE IS_VALID = 'TRUE'; \
                                               \
                                          SELECT  delivery_unit as du, vendor FROM \
                                          _SYS_REPO.DELIVERY_UNITS WHERE delivery_unit like \
                                          'HANA%'; SELECT  delivery_unit as du, vendor FROM \
                                          _SYS_REPO.DELIVERY_UNITS WHERE not delivery_unit \
                                          like 'HANA%'; END;"])?;
...
fn procedure_with_in_parameters(connection: &mut Connection) -> HdbResult<()> {
    ...
    test_utils::multiple_statements(connection,
        ...
        CREATE  PROCEDURE TEST_INPUT_PARS(IN \
                                          some_number INT, IN some_string NVARCHAR(20)) AS \
                                          BEGIN SELECT some_number AS \"I\", some_string AS \
                                          \"A\" FROM DUMMY; END;"])?;
...
fn procedure_with_in_and_out_parameters(connection: &mut Connection) -> HdbResult<()> {
    ...
    test_utils::multiple_statements(connection,
                                    vec!["\
        CREATE  PROCEDURE \
                                          TEST_INPUT_AND_OUTPUT_PARS(IN some_number INT, OUT \
                                          some_string NVARCHAR(40)) AS BEGIN some_string = 'my \
                                          first output parameter';
            SELECT \
                                          some_number AS \"I\" FROM DUMMY; END;"])?;
...
fn procedure_with_in_nclob_non_consuming(connection: &mut Connection) -> HdbResult<()> {
    ...
    test_utils::multiple_statements(connection,
                                    vec!["CREATE PROCEDURE TEST_CLOB_INPUT_PARS(IN some_string \
                                          NCLOB) AS BEGIN SELECT some_string AS \"A\" FROM \
                                          DUMMY; END;"])?;
