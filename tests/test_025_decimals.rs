...
pub fn test_025_decimals() {
    ...
    match impl_test_025_decimals(&mut log_handle) {
        Err(e) => {
...
fn impl_test_025_decimals(log_handle: &mut ReconfigurationHandle) -> HdbResult<()> {
...
fn evaluate_resultset(
    ...
    let resultset = connection.query("select f1, f2 from TEST_DECIMALS")?;
