...
fn impl_test_prepare() -> DbcResult<i32> {
...
fn prepare_insert_statement(connection: &mut Connection) -> DbcResult<()> {
    info!("test statement preparation and transactional correctness (auto_commit on/off, rollbacks)");
    test_utils::statement_ignore_err(connection, vec!("drop table TEST_PREPARE"));
    try!(test_utils::multiple_statements(connection, vec!(
        "create table TEST_PREPARE (F_S NVARCHAR(20), F_I INT)",
    )));
    ...
    // prepare & execute on first connection with auto_commit off, and rollback, do it again and commit
