...
pub fn test_select_and_deserialization() {
    ...
    match impl_test_select_and_deserialization() {
        Err(e) => {
            error!("test_select_and_deserialization() failed with {:?}", e);
            assert!(false)
        }
...
fn deser_option_into_option(connection: &mut Connection) -> HdbResult<()> {
    ...
    try!(connection.multiple_statements(vec!["create table TEST_OPT_OPT (F_S NVARCHAR(10), F_I \
                                              INT, F_D LONGDATE)",
                                             "insert into TEST_OPT_OPT (F_S) values('hello')",
                                             "insert into TEST_OPT_OPT (F_I) values(17)",
                                             "insert into TEST_OPT_OPT (F_D) \
                                              values('01.01.1900')"]));
...
fn deser_plain_into_plain(connection: &mut Connection) -> HdbResult<()> {
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["create table TEST_PLAIN_PLAIN (F_S NVARCHAR(10) \
                                               not null, F_I INT not null, F_D LONGDATE not \
                                               null)",
                                              "insert into TEST_PLAIN_PLAIN values('hello', \
                                               17, '01.01.1900')",
                                              "insert into TEST_PLAIN_PLAIN values('little', \
                                               18, '01.01.2000')",
                                              "insert into TEST_PLAIN_PLAIN values('world', \
                                               19, '01.01.2100')"]));
...
fn deser_plain_into_option(connection: &mut Connection) -> HdbResult<()> {
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["create table TEST_PLAIN_OPT (F_S NVARCHAR(10) \
                                               not null, F_I INT not null, F_D LONGDATE not \
                                               null)",
                                              "insert into TEST_PLAIN_OPT values('hello', 17, \
                                               '01.01.1900')",
                                              "insert into TEST_PLAIN_OPT values('little', 18, \
                                               '01.01.2000')",
                                              "insert into TEST_PLAIN_OPT values('world', 19, \
                                               '01.01.2100')"]));
...
fn deser_option_into_plain(connection: &mut Connection) -> HdbResult<()> {
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["create table TEST_OPT_PLAIN (F_S NVARCHAR(10), \
                                               F_I INT, F_D LONGDATE)"]));
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["insert into TEST_OPT_PLAIN values('hello', 17, \
                                               '01.01.1900')",
                                              "insert into TEST_OPT_PLAIN values('little', 18, \
                                               '01.01.2000')",
                                              "insert into TEST_OPT_PLAIN values('world', 19, \
                                               '01.01.2100')"]));
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["insert into TEST_OPT_PLAIN (F_I) values(17)"]));
    let resultset = try!(connection.query_statement("select * from TEST_OPT_PLAIN"));
...
fn deser_singleline_into_struct(connection: &mut Connection) -> HdbResult<()> {
    info!("deserialize a single-line resultset into a struct; test that multi-line resultsets \
           fail");
    test_utils::statement_ignore_err(connection, vec!["drop table TEST_SINGLE_LINE"]);
    try!(test_utils::multiple_statements(connection,
                                         vec!["create table TEST_SINGLE_LINE (O_S \
                                               NVARCHAR(10), O_I INT, O_D LONGDATE)",
                                              "insert into TEST_SINGLE_LINE (O_S) \
                                               values('hello')",
                                              "insert into TEST_SINGLE_LINE (O_I) values(17)",
                                              "insert into TEST_SINGLE_LINE (O_D) \
                                               values('01.01.1900')"]));
    ...
    let mut resultset = try!(connection.query_statement("select * from TEST_SINGLE_LINE where \
                                                         O_S = 'hello'"));
    ...
    let mut resultset = try!(connection.query_statement("select * from TEST_SINGLE_LINE"));
...
fn deser_singlevalue_into_plain(connection: &mut Connection) -> HdbResult<()> {
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["create table TEST_SINGLE_VALUE (O_S \
                                               NVARCHAR(10), O_I INT, O_D LONGDATE)",
                                              "insert into TEST_SINGLE_VALUE (O_S) \
                                               values('hello')",
                                              "insert into TEST_SINGLE_VALUE (O_I) values(17)",
                                              "insert into TEST_SINGLE_VALUE (O_D) \
                                               values('01.01.1900')"]));
    ...
    let resultset = try!(connection.query_statement("select TOP 3 O_S, O_D from \
                                                     TEST_SINGLE_VALUE"));
...
fn deser_singlecolumn_into_vec(connection: &mut Connection) -> HdbResult<()> {
    ...
    try!(test_utils::multiple_statements(connection,
                                         vec!["create table TEST_SINGLE_COLUMN (O_S \
                                               NVARCHAR(10), DUMMY int)",
                                              "insert into TEST_SINGLE_COLUMN (O_S, DUMMY) \
                                               values('hello', 0)",
                                              "insert into TEST_SINGLE_COLUMN (O_S, DUMMY) \
                                               values('world', 1)",
                                              "insert into TEST_SINGLE_COLUMN (O_S, DUMMY) \
    ...
    let resultset = try!(connection.query_statement("select O_S from TEST_SINGLE_COLUMN order \
                                                     by DUMMY asc"));
    ...
    let resultset = try!(connection.query_statement("select * from TEST_SINGLE_COLUMN order by \
                                                     DUMMY asc"));
    let typed_result: Vec<String> = match resultset.into_typed() {
        Ok(tr) => {
            error!("Typed Result: {:?}", tr);
            panic!("deserialization of a multi-column resultset into a Vec<plain field> did not \
                    fail");
            tr
        }
        Err(_) => {
