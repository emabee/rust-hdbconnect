...
pub fn test_021_date_serialization() -> HdbResult<()> {
    let mut loghandle = test_utils::init_logger("info,test_021_date_serialization=trace");

    let i = test_longdate(&mut loghandle)?;
    info!("longdate: {} calls to DB were executed", i);
...
fn test_longdate(_loghandle: &mut ReconfigurationHandle) -> HdbResult<i32> {
    info!(
        "test_longdate: verify that NaiveDateTime values match the expected string representation"
    );

    debug!("test_longdate: prepare the test data");
    ...
    {
        info!("test_longdate: test the conversion NaiveDateTime -> DB");
        ...
        info!("test_longdate: test the conversion DateTime<Utc> -> DB");
    ...
    {
        info!("test_longdate: test the conversion DB -> NaiveDateTime");
    ...
    {
        info!("test_longdate: prove that '' is the same as '0001-01-01 00:00:00.000000000'");
