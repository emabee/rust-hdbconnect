...
fn impl_test_longdate() -> DbcResult<i32> {
    ...
    try!(test_utils::multiple_statements(&mut connection, vec!(
    ...
    let selected_dates: Vec<LongDate> =
        try!(try!(connection.query_statement("select mydate from TEST_LONGDATE order by number asc")).into_typed());
    ...
    let rows_affected = try!(connection.dml_statement("insert into TEST_LONGDATE (number,mydate) values(77, '')"));
    assert_eq!(rows_affected,1);

    let selected_dates: Vec<LongDate> = try!(try!(
        connection.query_statement("select mydate from TEST_LONGDATE where number = 77 or number = 13"))
        .into_typed());
    debug!("selected_dates = {:?}",selected_dates);
